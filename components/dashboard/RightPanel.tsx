...
export function RightPanel({ user, activeTab = "news", onOpenSubmitModal }: RightPanelProps) {
    ...
    <aside className="w-full lg:w-80 bg-[#050A14] border-l border-white/5 p-6 overflow-y-auto custom-scrollbar">
      <div className="space-y-10">
        {activeTab === "news" ? (
                ...
                {trendingNews.length > 0 ? (
                  trendingNews.map((news, i) => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 p-3 bg-transparent hover:bg-accent-blue/8 transition-all group border-b border-accent-blue/20 last:border-b-0"
                    >
                      <span className="text-xs font-mono font-bold text-accent-blue min-w-[20px]">{i + 1}</span>
                      <span className="text-[11px] font-bold text-gray-400 group-hover:text-white transition-colors truncate flex-1">{news.headline}</span>
                      {i < 3 && <Flame size={12} className="text-accent-amber" />}
                    </button>
                  ))
                ) : (
          ...
          <>
            {/* Submit Article CTA - Articles Tab */}
            {onOpenSubmitModal && (
              <section>
                <button
              ...
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-white/[0.03] border border-white/5 rounded-xl">
                    <span className="text-xs font-mono font-bold text-accent-amber min-w-[20px]">{i}</span>
                    ...
                    <div className="flex-1">
                      <p className="text-[11px] font-bold text-white">@user{i}</p>
                      <p className="text-[9px] text-gray-500">{(i * 5000).toLocaleString()} views</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section>
              ...
              <div className="p-4 bg-white/[0.03] border border-white/5 rounded-xl">
                <h4 className="text-sm font-bold text-white leading-snug line-clamp-2 mb-2">
                  Understanding India's New Economic Policy Framework
                </h4>
