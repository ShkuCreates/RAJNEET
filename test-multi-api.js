// Test script to verify the multi-API implementation
const API_CONFIGS = {
  newsdata: {
    keys: [
  ...
  newsapi: {
    ...
    queryParams: "&language=en&pageSize=5&sortBy=publishedAt"
  },
  gnews: {
    keys: [
...
async function testAPI(apiName, query) {
  ...
  for (const apiKey of config.keys) {
    try {
      ...
      } else if ((apiName === 'newsapi' || apiName === 'gnews') && data.articles) {
...
async function testAllAPIs() {
  console.log('🚀 Testing Multi-API News Fetching System');
  console.log('===========================================');
  
  const query = "politics India";
  const allResults = [];
  const apiNames = ['newsdata', 'newsapi', 'gnews'];
  ...
  console.log('\n✅ Multi-API system test completed!');
}
